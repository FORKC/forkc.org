...
function cs_polyfill_dependencies( $deps ) {
	...
	// WordPress handles and minimum WordPress versions
	...
	foreach ($deps as $i => $dep) {
		...
		if ( ! isset( $polyfills[$dep] ) ) {
			continue;
		}

		$outdated = ! is_null( $polyfills[$dep] ) && version_compare( $polyfills[$dep], $wp_version, '>' );

		if ( ! wp_script_is( $dep, 'registered' ) || $outdated ) {
			$polyfill = "cs-$dep";
			if ( ! wp_script_is( $polyfill, 'registered' )) {
				wp_register_script( $polyfill, CS()->url( "assets/wp-compat/$dep.min.js" ), array(), CS()->version(), false );
			}
			$deps[$i] = $polyfill;
		}
	}
